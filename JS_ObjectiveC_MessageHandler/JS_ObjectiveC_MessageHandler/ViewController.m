...
- (void)createButton{
    ...
    [clickBtn setTitle:@"点击测试|原生调用JS" forState:UIControlStateNormal];
    ...
    clickBtn.frame = CGRectMake(0, 0, 150, 50);
    clickBtn.center = self.wkwebView.center;
    [self.wkwebView addSubview:clickBtn];
    [clickBtn addTarget:self action:@selector(click:) forControlEvents:UIControlEventTouchUpInside];
}
- (void)click:(UIButton*)sendor{
    [self.bridge callHandler:@"testJavaScriptFunction" data:@{ @"foo":@"before ready" } responseCallback:^(id responseData) {
#if DEBUG
...
//    [self.bridge setWebViewDelegate:self];
    ...
    [self.bridge registerHandler:@"testObjcFunction" handler:^(id data, WVJBResponseCallback responseCallback) {
        NSLog(@"testObjcFunction called: %@", data);
        responseCallback(@"Response from testObjcFunction");
    }];
    
    [self.bridge registerHandler:@"jsCallNativeTakePhoto" handler:^(id data, WVJBResponseCallback responseCallback) {
        NSLog(@"jsCallNativeTakePhoto called: %@", data);
        responseCallback(@"Response from jsCallNativeTakePhoto");
    }];
    
    [self.bridge registerHandler:@"scanClick" handler:^(id data, WVJBResponseCallback responseCallback) {
        NSLog(@"scanClick called: %@", data);
        responseCallback(@"Response from scanClick");
    }];
